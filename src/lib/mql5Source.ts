export const getStandaloneMql5Source = (backendUrl: string) => `//+------------------------------------------------------------------+
//|                                              SmartTraderEA.mq5  |
...
//--- Inputs
input string   InpBackendUrl            = "http://127.0.0.1:3000";             // Backend Gateway URL (Localhost IP)
...
input int      InpMinimumScore          = 70;                                 // Minimum Setup Score to Qualify (0-100)
...
input int      InpHeartbeatIntervalSec  = 5;                                  // Sync / Heartbeat Interval (Seconds)
...
void SendSnapshotAndEvaluate()
  {
   ...
   if(g_httpClient != NULL && g_httpClient.SendMarketSnapshot(jsonSnapshot, responseStr))
     {
      PrintFormat("[MT5->AI] Market Snapshot Sent: %s | Bid: %.2f | Ask: %.2f | ATR: %.2f | Score: %d", _Symbol, bid, ask, realAtr, (int)87);
     }
  }
